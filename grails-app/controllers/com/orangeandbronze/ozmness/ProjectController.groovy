...
class ProjectController {
	...
	def showPossibleCollaborators =  {
		def projectInstance = Project.get(params.id)
		if(projectInstance){
			def projectLead = Employee.get(projectInstance.lead.id)
			def possibleCollaborators = Employee.list() - projectInstance.collaborators - projectLead
			return  [projectInstance: projectInstance, possibleCollaborators: possibleCollaborators]
		}
	}
	
	def addCollaborator ={
		int collaboratorID = params.collaborator.id
		def employeeInstance = Employee.get(collaboratorID)
		def projectInstance = Project.get(params.id)
